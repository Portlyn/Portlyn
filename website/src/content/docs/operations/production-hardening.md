...
description: Settings to tighten before you put Portlyn on the internet.
---
...
- Turn bootstrap login off (`BOOTSTRAP_ADMIN_ENABLED=false`) once setup is done. Fresh installs ship with it on so the dashboard is reachable before a domain exists, and it only answers local requests unless you also set `BOOTSTRAP_ADMIN_ALLOW_REMOTE=true`.
